...
// ── Instructor: Grade / Re-grade Modal ────────────────────────────────────────
...
export default function AssignmentsPage() {
    ...
    // Scoped data
    const myCourses = isInstructor ? getInstructorCourses(currentUser.id).filter(c => c.status === 'published') : [];
    const myAssignments = isInstructor
        ? getInstructorAssignments(currentUser.id)
        : getStudentAssignments(currentUser.id);
    const mySubmissions = isInstructor
        ? getInstructorSubmissions(currentUser.id)
    ...
    const pendingCount = isInstructor ? mySubmissions.filter(s => s.status === 'pending').length : 0;
                            ...
                            <div key={a.id} className="card" style={{ display: 'flex', alignItems: 'flex-start', gap: 20, padding: '20px 24px' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ fontSize: 13, color: 'var(--text-muted)', display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: a.description ? 8 : 0 }}>
                                        ...
                                        {isInstructor && <span>{a.submissions} submitted · {a.graded} graded</span>}
                                    </div>
                                    {a.description && (
                            ...
                            {mySubmissions.map(s => (
                                <div key={s.id} className="card" style={{ display: 'flex', alignItems: 'flex-start', gap: 20, padding: '18px 24px' }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <div style={{ fontSize: 13, fontWeight: isInstructor ? 400 : 600, color: isInstructor ? 'var(--text-secondary)' : 'var(--text-primary)', marginBottom: 4 }}>
                                            {s.assignmentTitle}
                                        </div>
