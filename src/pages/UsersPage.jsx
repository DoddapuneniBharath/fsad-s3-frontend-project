...
const ROLE_OPTIONS = ['Admin', 'Instructor', 'Student', 'CONTENT_CREATOR'];
const ROLE_LABELS = { Admin: 'Admin', Instructor: 'Instructor', Student: 'Student', CONTENT_CREATOR: 'Content Creator' };
const ROLE_ICONS = { Admin: <Shield size={14} />, Instructor: <Star size={14} />, Student: <User size={14} />, CONTENT_CREATOR: <Palette size={14} /> };
const ROLE_COLORS_MAP = {
    ...
    CONTENT_CREATOR: { bg: 'rgba(245,158,11,0.15)', color: '#fbbf24' },
};

function UserModal({ user, onClose, onSave }) {
    const [form, setForm] = useState(user || { name: '', email: '', role: 'Student', status: 'active' });
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
            <div className="modal" style={{ maxWidth: 480 }}>
                ...
                <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                    ...
                    <button className="btn btn-primary" onClick={() => { if (form.name && form.email) { onSave(form); onClose(); } }}>
        ...
        <div className="animate-fadeIn">
            ...
            <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: 24 }}>
                {ROLE_OPTIONS.map(role => {
                    ...
                    return (
                        <div key={role} className="stat-card" style={{ cursor: 'pointer' }} onClick={() => setFilterRole(filterRole === role ? 'All' : role)}>
                            ...
                            <div className="stat-label">{ROLE_LABELS[role]}s</div>
                        </div>
                    );
                })}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <div style={{ marginLeft: 'auto', color: 'var(--text-muted)', fontSize: 13 }}>{filtered.length} results</div>
            </div>
            ...
            {showModal && (
                <UserModal
                    ...
                    onSave={data => editUser ? updateUser(editUser.id, data) : addUser(data)}
                />
            )}
